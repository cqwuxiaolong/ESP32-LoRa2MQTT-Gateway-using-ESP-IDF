...
void SSD1306_SetDisplayStartLine( struct SSD1306_Device* DeviceHandle, int Line ) {
    ...
    SSD1306_WriteCommand( DeviceHandle, 0x40                                       // changed to 0x40
...
static void SetCOMPinConfiguration( struct SSD1306_Device* DeviceHandle, uint32_t RemapCFG, uint32_t PinCFG, int ScanDir ) {
    ...
    SSD1306_WriteCommand( DeviceHandle,  ( uint8_t ) ( RemapCFG | PinCFG | BIT( 1 ) )  /*0x12*/); 
...
void SSD1306_SetDisplayClocks( struct SSD1306_Device* DeviceHandle, uint32_t DisplayClockDivider, uint32_t OSCFrequency ) {
    ...
    SSD1306_WriteCommand( DeviceHandle, /* ( ( OSCFrequency << 4 ) | DisplayClockDivider )  */ 0xF0 );  // change to 0xF0
}
...
void setBoundaries(struct SSD1306_Device* DeviceHandle, int width,int height){
    const int x_offset = (128 - width) / 2;
    SSD1306_SetColumnAddress( DeviceHandle, x_offset, x_offset + (DeviceHandle->Width - 1));
...
static bool SSD1306_Init( struct SSD1306_Device* DeviceHandle, int Width, int Height ) {
    ...
    SSD1306_SetMuxRatio( DeviceHandle,   0x3F /* DeviceHandle->Height - 1 */);  // changed
    SSD1306_SetDisplayOffset( DeviceHandle, 0x00 );  // same
    SSD1306_SetDisplayStartLine( DeviceHandle, 0 );
    ...
    SSD1306_SetContrast( DeviceHandle, 0xCF );
    ...
    SSD1306_WriteCommand( DeviceHandle, 0xD9 ); //setprecharge
    SSD1306_WriteCommand( DeviceHandle, 0xF1 );

    SSD1306_WriteCommand( DeviceHandle,  0xDB ); //setvcomdetect
    SSD1306_WriteCommand( DeviceHandle, 0x40 );

    SSD1306_EnableDisplayRAM( DeviceHandle );
    SSD1306_DisplayOn( DeviceHandle );
    SSD1306_Update( DeviceHandle );
    setBoundaries(DeviceHandle, DeviceHandle->Width,DeviceHandle->Height);
    return true;
}
